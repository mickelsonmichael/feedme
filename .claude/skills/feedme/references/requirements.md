...
| ID            | Requirement                                                                                                                                                                                                                                                                              |
| ------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| REQ-MEDIA-001 | [updated] Tapping an expanded image (rendered by `ExpandedFeedImage`) shall open a fullscreen modal displaying the image at full viewport size with a dark backdrop. The modal shall include a close button (×) in the top-right corner. Tapping the close button shall dismiss the modal. Tapping the backdrop while not zoomed shall dismiss the modal; tapping while zoomed shall reset the zoom. On web, pressing Escape shall also dismiss the modal. NSFW-blurred images retain their blur in fullscreen. The modal supports pinch-to-zoom (scale 1×–5×) with pan when zoomed. Releasing a pinch below 1.1× snaps the image back to 1×. On native, the modal unlocks device orientation so the user can rotate to landscape; closing the modal restores portrait lock. |
...
| REQ-LOAD-001  | [added] Wherever the app indicates that posts are being fetched — the background sync banner, the full-screen loading state, and the load-more footer — it shall show an animated loader of a head chewing through a queue of dots (`ChompingLoader`), replacing the previous three-bouncing-dots indicator. Exactly one dot shall be consumed per bite of the jaws, and each dot shall travel into the mouth and be covered by the closing jaws rather than disappearing short of them. The head shall render as a solid shape with no seam visible along its diameter when the mouth is shut. Every moving part shall be driven from a single animation loop, so the parts cannot drift out of step with each other however long a refresh runs. The loader shall scale as a whole with its `size` prop, keeping its proportions. |
...
| REQ-FEEDLIST-004 | The user shall be able to filter items to show all, unread only, or saved only. See REQ-FEEDLIST-026 for how the unread filter interacts with paging.                                                                                                                                                                                                                                                                                                                                                                                            |
...
| REQ-FEEDLIST-024 | [modified] In single layout on native platforms (Android/iOS, excluding web and mobile web), swiping left on the post content advances to the next post (same behavior as the Next button, including its view-time-session and load-more semantics); swiping right returns to the previous post (same behavior as the Previous button, including that it does not record a view-time session). Swipes that don't clear a minimum distance/velocity threshold, or that are more vertical than horizontal, are ignored so normal scrolling is unaffected. The post card follows the finger horizontally in real time while dragging; releasing past the threshold slides the revealed post in from the swiped-from edge, while a rejected swipe (below threshold, wrong direction at a boundary, or a next-swipe that only triggers a load-more) springs the card back to center. |
| REQ-FEEDLIST-025 | [added] In single layout, the post header shall display the feed's favicon-style icon next to the feed name/date byline, derived from the feed's URL. If no icon can be derived or the image fails to load, the icon is omitted entirely (no placeholder). |
| REQ-FEEDLIST-026 | [added] When the unread filter is active, pages shall be queried unread-only rather than filtered client-side after loading. A page whose posts the user has already read shall therefore never collapse to an empty list while unread posts remain unloaded: launching the app after reading the top of the feed shall show the previously fetched unread posts, with the refresh running behind the background sync banner. "All caught up!" shall be shown only when the library genuinely holds no unread posts for the current scope. Changing the filter re-queries page 0. Posts held on screen by mark-as-read-on-scroll (see REQ-FEEDLIST-007) shall survive an unread-only re-query, and paging offsets shall count only query-sourced rows so those held posts cannot push the next page past unloaded unread posts. |
...
| REQ-FEEDITEM-009 | [added] The "..." overflow menu shall include an "Edit Feed" option that resolves the post's feed and navigates to FeedDetailScreen for it. Disabled when the post has no live database item id. If the feed can no longer be found (e.g. deleted), an error alert is shown instead of navigating. Pressing the back button on FeedDetailScreen in this scenario returns to this same post rather than the Feeds tab. |
| REQ-FEEDITEM-010 | [added] The header shall display the feed's favicon-style icon next to the feed name/date byline, derived from the feed's URL. If no icon can be derived or the image fails to load, the icon is omitted entirely (no placeholder). |
| REQ-FEEDITEM-011 | [added] The article body shall be laid out for long-form reading: a line height of 1.6x the body font size, and a gap after each paragraph, list, blockquote and code block of 2x the body font size — i.e. visibly larger than the gap between lines, so a paragraph break cannot be mistaken for a wrapped line. Headings take the same gap above them and a tighter 1.3x line height. This applies identically to the HTML article body and the plain-text article body, and to both the reader and a post card's expanded content. |
| REQ-FEEDITEM-012 | [added] Paragraph breaks shall be normalized before rendering so that every paragraph is separated by the same gap regardless of how the feed wrote it: a run of two or more consecutive `<br>` elements inside a paragraph becomes a paragraph boundary, spacer elements holding only whitespace/`&nbsp;`/`<br>` are removed, and plain-text content is split into paragraphs on blank lines. A single `<br>` remains an in-paragraph line break. |
...
| REQ-FEEDDETAIL-012 | [added] When opened via the "Edit Feed" action from a post's overflow menu (FeedItemScreen), the header/mobile back button and the desktop-web back button shall return to that same post instead of the Feeds tab. Deleting the feed while in this scenario also returns to the originating post. |
